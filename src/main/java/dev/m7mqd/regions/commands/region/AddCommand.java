...
public class AddCommand extends SubCommand {
    ...
    public AddCommand(RegionService regionService, Plugin plugin) {
        super("add", "regions.add");
    ...
    public void run(CommandSender sender, String[] args) {
        if (args.length < 2) {
            Messenger.send(sender, "<red>Usage: /region add <name> <username>");
            return;
        }
        ...
        plugin.getServer().getScheduler().runTaskAsynchronously(plugin, () ->{
            ...
            Messenger.send(sender, "<green>Added <player> to region <region>.",
